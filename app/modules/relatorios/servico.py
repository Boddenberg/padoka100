...
def buscar_resumo_do_dia_de_venda(dia_de_venda_id: UUID) -> dict:
    ...
    itens_venda = []
    if venda_ids:
        itens_venda = client.table("itens_venda").select("*").in_("venda_id", venda_ids).execute().data

    produtos = _montar_resumos_dos_produtos(itens_producao, itens_venda)
...
def _montar_resumos_dos_produtos(itens_producao: list[dict], itens_venda: list[dict]) -> list[dict]:
    ...
    for item in itens_venda:
        ...
        resumo["quantidade_sobra"] = resumo["quantidade_produzida"] - resumo["quantidade_vendida"]
...
def _somar_produtos(produtos: list[dict]) -> dict:
    return {
        ...
        "faturamento_bruto": sum((produto["faturamento_bruto"] for produto in produtos), Decimal("0")),
